...
static inline uint32_t rotlu32(uint32_t arg, unsigned bits) {
  // __builtin_stdc_rotate_left is not available in WCH-gcc yet :-(
  return (uint32_t)((arg << (bits % 32)) | (arg >> ((-bits) % 32)));
}
